...
export function LoginPage() {
  ...
  async function submit(values) { try { setError(''); await login({ email: values.email, password: values.password }); navigate(location.state?.from?.pathname || '/groups', { replace: true }) } catch (requestError) { setError(requestError.response?.data?.message || requestError.message || 'Login failed. Please try again.') } }
